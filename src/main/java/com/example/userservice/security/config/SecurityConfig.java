...
//                .username("user")
//                .password("$2a$12$q6l7kFilCIIaNVoMRew94OJJYYS0XQJgVw93JjW4bYoDd5wFJvgiO")
//                .roles("USER")
//                .build();
//
        ...
        return context -> {
            if (context.getPrincipal() != null && context.getTokenType().getValue().equals("access_token")) {
                ...
                if (principalObj instanceof CustomUserDetails customUserDetails) {
                    // Add claims from CustomUserDetails
                    context.getClaims().claim("user_name", customUserDetails.getUsername());
                    context.getClaims().claim("mobile", customUserDetails.getMobile());
                    context.getClaims().claim("user_id", customUserDetails.getUserId());
